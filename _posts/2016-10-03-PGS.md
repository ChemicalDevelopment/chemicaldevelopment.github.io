...
The `1`s mean that the index is prime, so count from the left:

`0`, `1`, `2`, and you'll see that number is `1`, which indicates that `2` is prime.
...
And for memory usage, the first approach